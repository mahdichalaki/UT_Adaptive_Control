...
colored_noise(3:N) = numc(1)*noise(3:N)+ numc(2)*noise(2:N-1);
...
%     uf(k) = ContrINPUT(k) - [uf(k-1) uf(k-2)]*[numc(2) numc(3)]';
    uf(k) = ContrINPUT(k) - 0.2*ContrINPUT(k-1) - uf(k-1)*numc(2);
    yf(k) = y(k) - 0.2*ContrINPUT(k-1) - yf(k-1)*numc(2);
