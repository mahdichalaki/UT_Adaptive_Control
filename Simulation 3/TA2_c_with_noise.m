...
u(i,:,1) = 1 + u_noise(i);
% u(i,:,1) = 1;
u(i,:,2) = (-1)^ceil(i/5000) + u_noise(i);
% u(i,:,2) = (-1)^ceil(i/25000);
...
% function [theta_hat, Cost] = RLS(i,u,theta,theta_hat_init,noise,N,P,Disturbance,A,B,Am,Ao)
    ...
    y(1) = 0;
    y(2) = [-y(1) -y_zero -y_minus1 u(1,:,i) u_zero u_minus1] * theta + colored_noise(2);
    y(3) = [-y(2) -y(1) -y_zero u(2,:,i) u(1,:,i) u_zero] * theta + colored_noise(3);
    
    Phi = zeros(N,length(theta)+1);
    Phi(2,1:end-1) = [-y(1) -y_zero -y_minus1 u(1,:,i) u_zero u_minus1];
    Phi(3,1:end-1) = [-y(2) -y(1) -y_zero u(2,:,i) u(1,:,i) u_zero];
    ...
    for k = 4:N
        ...
        y(k)=[-y(k-1) -y(k-2) -y(k-3) ContrINPUT(k-1) ContrINPUT(k-2) ContrINPUT(k-3)] * theta + colored_noise(k);
        Phi(k,:)=[-y(k-1) -y(k-2) -y(k-3) ContrINPUT(k-1) ContrINPUT(k-2) ContrINPUT(k-3) epsilon(k-1)];
        epsilon(k) = y(k) - Phi(k,:) * theta_hat(:,k-1);
        P = inv(inv(P)+Phi(k,:)'*Phi(k,:));
%         P = P - P*Phi(k,:)'*Phi(k,:)*P/(1+Phi(k,:)*P*Phi(k,:)');
        K = P * Phi(k,:)';
