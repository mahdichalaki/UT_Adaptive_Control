...
[theta_hat_pa,cost_pa] = PA(u,theta,theta_hat_init,noise,N,P,alpha,gamma);

%% Function

function [theta_hat, Cost] = PA(u,theta,theta_hat_init,noise,N,P,alpha,gamma)
    ...
    for k = 2:4
        ...
        theta_hat(:,k) = theta_hat(:,k-1) + (gamma /(alpha+Phi(k,:)*Phi(k,:)')) * Phi(k,:)' * (y(k) - Phi(k,:)*theta_hat(:,k-1));
