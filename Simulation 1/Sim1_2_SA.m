...
[theta_hat_sa,cost_sa] = SA(u,theta,theta_hat_init,noise,N,P,alpha,gamma);

%% Function

function [theta_hat, Cost] = SA(u,theta,theta_hat_init,noise,N,P,alpha,gamma)
    ...
    for k = 2:4
        A = Phi(1:k,:)*Phi(1:k,:)';
        P = 1/sum(diag(A));
        theta_hat(:,k) = theta_hat(:,k-1) + P * Phi(k,:)' * (y(k) - Phi(k,:)*theta_hat(:,k-1));
