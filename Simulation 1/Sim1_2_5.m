...
N = 1000000;
theta_hat_init = zeros(length(theta)+1,N);
...
colored_noise(2:N) = noise(2:N)+ 0.01 * noise(1:N-1);
...
function [theta_hat, Cost] = RLS(u,theta,theta_hat_init,noise,N,P)
    ...
    Phi(2,1:end-1) = [-y(1) -y_zero -y_minus1 -y_minus2 u(1) u_zero u_minus1 u_minus2];
    Phi(3,1:end-1) = [-y(2) -y(1) -y_zero -y_minus1 u(2) u(1) u_zero u_minus1];
    Phi(4,1:end-1) = [-y(3) -y(2) -y(1) -y_zero u(3) u(2) u(1) u_zero];
...
%         P = P - P*Phi(k,:)'*inv(1+Phi(k,:)*P*Phi(k,:)')*Phi(k,:)*P;
    ...
    for k = 5:N
        ...
        Phi(k,:)=[-y(k-1) -y(k-2) -y(k-3) -y(k-4) u(k-1) u(k-2) u(k-3) u(k-4) epsilon(k-1)];
        epsilon(k) = y(k) - Phi(k,:) * theta_hat(:,k-1);
%         P = P - P*Phi(k,:)'*inv(1+Phi(k,:)*P*Phi(k,:)')*Phi(k,:)*P;
        P = inv(inv(P)+Phi(k,:)'*Phi(k,:));
        theta_hat(:,k) = theta_hat(:,k-1) + P * Phi(k,:)'*epsilon(k);
