...
clear;
sysC  = zpk([-30,0.1], [-1.3,-2.5,1.1], 0.2);
BW = bandwidth(sysC); 
...
%     theta_hat_new = els_solver.update_ELS(error, phi_d0_filtered);
    theta_hat_new = rls_solver.update_RLS(error, phi_d0_filtered);
...
%     [R_estimated, S_estimated] = remove_common_roots(BR_estimated,BS_estimated,precision);
    R_estimated = deconv(BR_estimated, B);
    S_estimated = deconv(BS_estimated, B);

    assert(length(R_estimated )==3)
