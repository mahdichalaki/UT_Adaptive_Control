...
for i = 3:4
    title_text = "b_%d";
    subplot(ceil(length(theta_hat_toPlot(1,:)))/4,2,i-2);
    plot(1:num_samples, theta_hat_toPlot(:,i-1), 'DisplayName','Estimated parameter')
    hold on;
    plot(1:num_samples, ones([num_samples,1]) * theta_real(i-1) , 'DisplayName','Actual parameter')
    title(sprintf(title_text, i-1));
    legend()
%     legend('Location','best');
%     xlabel("sample number")
