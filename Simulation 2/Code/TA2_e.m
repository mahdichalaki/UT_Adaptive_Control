...
N = 10000;
theta_hat_init = 0.5*ones(length(theta),N);
theta_hat_init(1:3,:) = 0.01*ones(3,N);
...
% function [theta_hat, Cost] = RLS(i,u,theta,theta_hat_init,noise,N,P,Disturbance,A,B,Am,Ao)
    ...
    for k = 4:N
        ...
        y(k)=[-y(k-1) -y(k-2) -y(k-3) ContrINPUT(k-1)+Disturbance(k-1) ContrINPUT(k-2)+Disturbance(k-2) ContrINPUT(k-3)+Disturbance(k-3)] * theta;
        ...
        R = conv([1 0],R) - [0 (sum(R)/sum(B_hat))*B];
        S = conv([1 0],S) + (sum(R)/sum(B_hat))*A;

        ContrINPUT(k) = T * [u(k-1,:,i) u(k-2,:,i) u(k-3,:,i)]'...
                      + S * [-y(k) -y(k-1) -y(k-2) -y(k-3)]'...
                      - R(2:end) * [ContrINPUT(k-1) ContrINPUT(k-2) ContrINPUT(k-3)]';
...
%                       - R(2:end) * [ContrINPUT(k-1) ContrINPUT(k-2)]';
    ...
    figure()
    hold on
    plot (OUTPUT (:,1,:)*1.5, 'b', 'linewidth',1)
